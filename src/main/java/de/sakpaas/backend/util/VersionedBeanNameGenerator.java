...
public class VersionedBeanNameGenerator implements BeanNameGenerator {
    ...
    public static final BeanNameGenerator DEFAULT_BEAN_NAME_GENERATOR = new DefaultBeanNameGenerator();

    @Override
    public String generateBeanName(BeanDefinition beanDefinition, BeanDefinitionRegistry beanDefinitionRegistry) {
        if(! beanDefinition.getBeanClassName().startsWith(BASE_PACKAGE))
            return DEFAULT_BEAN_NAME_GENERATOR.generateBeanName(beanDefinition, beanDefinitionRegistry);
        ...
        Optional<String> version = Arrays.stream(split)
                ...
                .findAny();
        String beanName = Arrays.stream(split).reduce((first, second) -> second).get() + version.orElse("");
        System.out.println(beanDefinition.getBeanClassName() + ": " + beanName);
        return beanName;
    }
}